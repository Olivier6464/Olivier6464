...
function affichage() {
  ...
  const egal = "=";
  // <--donnees a tester-->
  const douleurRepos = document.getElementById("douleurRepos").value;
  const douleurMob = document.getElementById("douleurMob").value;
  ...
  bil +=
    ...
    douleurRepos +
    "/4";
  bil += rtl + mobilisation + " " + douleurMob + "/4";
  if (douleurRepos !== "0" || douleurMob !== "0") {
    bil += rtl + "Type: " + getSelectionsListe("typeDouleur");
    bil +=
      rtl + "Localisation: " + document.getElementById("localisation").value;
  }
